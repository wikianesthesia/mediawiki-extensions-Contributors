...
class ApiQueryCnContributors extends ApiQueryBase {
    ...
    public function execute() {
        ...
        if( !Contributors::userCanGetContributors( Title::newFromText( $params['titles'] ) ) ) {
            $this->dieWithError( 'contributors-permissiondenied' );
        }
    ...
    private function buildDbQuery( array $params ) {
        ...
        $this->addFields(
            [
                ...
                'Number of characters added' => 'cn_characters_added',
                'Date of first edit' => 'cn_first_edit',
                'Date of last edit' => 'cn_last_edit'
            ]
        );
        $this->addOption( 'ORDER BY', Contributors::getDefaultSort() . ' DESC' );
        $this->addJoinConds( [
            'page' => [
                'LEFT JOIN',
                [ 'cn_page_id = page_id' ]
